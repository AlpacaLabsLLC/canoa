...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "canoa_status",
            description: "Return whether the designer is signed in to Canoa, with their email and master sheet info if so. Cheap local lookup — no network call. The /canoa skill calls this first to decide whether onboarding steps are needed.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "canoa_signup",
            description: "Sign the designer in to Canoa via Google OAuth. Opens their default browser to Google's consent screen (Sheets + Drive scopes), waits up to 5 minutes for them to complete the flow, and persists the resulting user_id locally so future tool calls are bound to this account. Called by the /canoa onboarding prompt when canoa_status reports not signed in.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "canoa_signout",
            description: "Sign the designer out of Canoa. Removes the local credentials file and clears the in-process user binding so subsequent canoa_chat calls error until the designer signs in again. Use when the designer says 'sign out', 'log out', 'switch accounts', or similar.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "canoa_attach_sheet",
            description: "Attach a Google Sheet as the designer's master schedule. Validates that Canoa has access to it (the designer must have shared it with the signed-in Google account, OR enabled link-based editing) and stores its title locally so the agent knows which sheet to read/write. Called by the /canoa onboarding prompt after signup, and any time the designer says 'attach my sheet', 'connect my schedule', etc.",
            inputSchema: {
                type: "object",
                properties: {
                    url: {
...
async function handleAttachSheet(rawArgs) {
    if (!activeUserId) {
        return textResult("Can't attach a sheet — no designer is signed in. Run canoa_signup first.", true);
    }
    const args = rawArgs;
    ...
    console.error(`[canoa-mcp] sheet attached id=${sheet.google_sheet_id.slice(0, 8)}... name="${sheet.sheet_name}" tabs=${tabs.length}`);
    return textResult(`Attached "${sheet.sheet_name}" as the master schedule.${tabs.length ? ` Tabs: ${tabs.join(", ")}.` : ""} Canoa can now read this sheet (and, in Wedge 3, write specs back to it).`);
}
async function handleSignup() {
    let startBody;
    try {
        const resp = await fetch(`${API_BASE}/signup/start`, { method: "POST" });
        startBody = (await resp.json());
        if (!resp.ok) {
        ...
        const detail = err instanceof Error ? err.message : String(err);
        return textResult(`Could not reach ${API_BASE}/signup/start. Is wrangler pages dev running? (${detail})`, true);
    }
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        if (statusBody.status === "complete" && statusBody.user_id) {
            ...
            history.length = 0;
            console.error(`[canoa-mcp] signup complete user_id=${statusBody.user_id} email=${statusBody.email}`);
            return textResult(`Signed in to Canoa as ${statusBody.email ?? statusBody.user_id}.\n\n` +
    ...
    return textResult(`Sign-in timed out after 5 minutes without completion. If you finished the OAuth flow, check the browser tab for an error. Otherwise, run canoa_signup again.`, true);
}
async function handleChat(rawArgs) {
    const args = rawArgs;
    ...
    if (!activeUserId) {
        return textResult("No designer is signed in to Canoa yet. Run canoa_signup first to complete Google OAuth, then retry this message.", true);
    }
    let resp;
    try {
    ...
    try {
        ...
        return textResult(`Non-JSON response from /api/chat (status ${resp.status}): ${bodyText.slice(0, 500)}`, true);
    }
    if (!resp.ok) {
    ...
    console.error(`[canoa-mcp] turn ok — tools=${tools.length} cost_cents=${body.cost_cents ?? "?"} remaining_cents=${body.remaining_trial_cents ?? "?"} latency_ms=${body.latency_ms ?? "?"}`);
...
console.error(`[canoa-mcp] connected — api_base=${API_BASE} active_user=${activeUserId ?? "<none>"} ${activeEmail ? `(${activeEmail}) ` : ""}persona_chars=${personaBody.length}`);
